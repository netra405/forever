...
// Debug: return latest transaction (admin only)
const latestTransaction = async (req, res) => {
    try {
        const tx = await Transaction.findOne({}).sort({ createdAt: -1 }).limit(1);
        if (!tx) return res.json({ success: false, message: 'No transactions found' });
